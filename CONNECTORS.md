...
An Earthdata Login is needed only to retrieve NASA data; USGS water data needs no account, and searching CMR through this connector needs none either. earthaccess looks for the credential in the environment first (an `EARTHDATA_TOKEN`, or username and password variables), then `~/.netrc` (machine `urs.earthdata.nasa.gov`, chmod 600), then an interactive prompt. Credentials never appear in any repo, in any of those forms (SPEC §5.8).
