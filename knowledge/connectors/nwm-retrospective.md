...
sources:
  ...
  - id: unserved-stores
    resource: https://noaa-nwm-retrospective-3-0-pds.s3.amazonaws.com/Hawaii/zarr/chrtout.zarr/.zmetadata
    title: "The consolidated metadata of the Hawaii chrtout store, and of the PR one at https://noaa-nwm-retrospective-3-0-pds.s3.amazonaws.com/PR/zarr/chrtout.zarr/.zmetadata, read 2026-09-15: both streamflow arrays carry the compressor id blosc with cname lz4; the Hawaii time axis is in minutes since 1994-01-01T00:15:00 and the PR one in hours since 2008-01-01T01:00:00, both on the proleptic Gregorian calendar"
  - id: chunk-sizes
...
runs between 282 and 606 m3/s: the model holds a near-constant value
below Glen Canyon Dam where the gauge records the
...
**Layout (read 2026-09-15).** The bucket lists four domains, CONUS,
Alaska, Hawaii and PR; CONUS carries a netcdf tree and a zarr tree,
and Alaska carries the same zarr stores.[^bucket-layout] The CONUS channel
...
record runs to 2023-02-01T00:00, the first calendar day has 23
hours and the last one, 2023-02-01, has one.[^zmetadata]
`feature_id` is the NHDPlus v2 ComID within CONUS and an arbitrary
reach id outside it; `gage_id` is a 15-byte string per reach, blank
on all but 8,660 of them, naming the USGS gauge the model assigns to
the reach.[^zmetadata][^lees-ferry-probe] A
...
the hours counted, so a partial day is visible.[^server] A `domain`
parameter names the store, CONUS by default or Alaska; Hawaii and PR
are refused by name before any request, because their chrtout
stores are blosc/lz4 compressed and Hawaii's time axis is in minutes
since 1994-01-01T00:15, which the tool does not
decode.[^server][^unserved-stores]
...
[^unserved-stores]: the Hawaii and PR chrtout stores' consolidated metadata, read 2026-09-15
